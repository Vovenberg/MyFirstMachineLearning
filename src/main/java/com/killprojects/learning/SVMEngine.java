...
import com.killprojects.learning.models.kernel.SVC_Q;
import com.killprojects.learning.models.kernel.SVR_Q;
import com.killprojects.learning.models.kernel.solver.Solver;
import com.killprojects.learning.models.kernel.solver.Solver_NU;
import com.killprojects.learning.models.main.*;
    ...
    private double svm_svr_probability(SVNProblem prob, SVMParams param) {
        ...
        for (i = 0; i < prob.l; i++)
            if (Math.abs(ymv[i]) > 5 * std)
                ...
                mae += Math.abs(ymv[i]);
        mae /= (prob.l - count);
        info("Prob. main for test data: target value = predicted value + z,\nz: Laplace distribution e^(-|z|/sigma)/(2sigma),sigma="
                + mae + "\n");
        return mae;
    }
...
//            // build output
//
//            main.nr_class = nr_class;
//
//            main.label = new int[nr_class];
//            for (i = 0; i < nr_class; i++)
//                main.label[i] = label[i];
//
//            main.rho = new double[nr_class * (nr_class - 1) / 2];
//            for (i = 0; i < nr_class * (nr_class - 1) / 2; i++)
//                main.rho[i] = f[i].rho;
//
//            if (param.probability == 1) {
//                main.probA = new double[nr_class * (nr_class - 1) / 2];
//                main.probB = new double[nr_class * (nr_class - 1) / 2];
//                for (i = 0; i < nr_class * (nr_class - 1) / 2; i++) {
//                    main.probA[i] = probA[i];
//                    main.probB[i] = probB[i];
//                }
//            } else {
//                main.probA = null;
//                main.probB = null;
...
//
//            main.sv_coef = new double[nr_class - 1][];
//            for (i = 0; i < nr_class - 1; i++)
//                main.sv_coef[i] = new double[nnz];
//
//            p = 0;
//            for (i = 0; i < nr_class; i++)
//                for (int j = i + 1; j < nr_class; j++) {
...
//                    for (k = 0; k < ci; k++)
//                        if (nonzero[si + k])
//                            main.sv_coef[j - 1][q++] = f[p].alpha[k];
//                    q = nz_start[j];
//                    for (k = 0; k < cj; k++)
//                        if (nonzero[sj + k])
//                            main.sv_coef[i][q++] = f[p].alpha[ci + k];
//                    ++p;
//                }
//        }
        return model;
        ...
        while (true) {
            ...
            if (cmd.startsWith("labels")) {
                ...
                if (i == svm_type_table.length) {
                    System.err.print("unknown models type.\n");
                    return null;
                }
                ...
                System.err.print("unknown text in main file: [" + cmd + "]\n");
                return null;
            }
        }
